...
function createMap() {
  ...
  for (var i = 0 ; i<split_values.length - 1; i+=3){
    ...
    var latLng = {lat: parseFloat(split_values[i])-0.0001, lng: parseFloat(split_values[i+1])};
    ...
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      for (var i = 0, len = result.routes.length; i<len; i++){
        directionsDisplay = new google.maps.DirectionsRenderer({
          directions: result,
          map: map,
          routeIndex: sorted_Routes[result.routes.length-1-i].index,
          polylineOptions: {
            strokeColor: getColorFromMag(routes_Danger_Array[result.routes.length-1-i].danger),
            strokeWeight: 5,
            strokeOpacity: 1
          }
        });
